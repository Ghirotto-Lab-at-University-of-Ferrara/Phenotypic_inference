...
for (j in 1:1000) {
  pescate <- data.frame(matrix(ncol = 1, nrow = nrow(genPosteriors)))
  row.names(pescate) <- rownames(genPosteriors)
  colnames(pescate) <- "Genotipo pescato"
  colnames(pescate) <- "Pescata"
  pescate$Num <- genPosteriors$Num
  pescate$Rs_allele <- genPosteriors$Rs_allele
  pescate$Strand <- genPosteriors$Strand
  pescate$Position <- genPosteriors$Position
  for (i in 1:nrow(genPosteriors)) {
    ...
    pescate[i, "Genotipo pescato"] <- genotipo_i
    if (is.na(genotipo_i)) {
      pescate[i, "Pescata"] <- NA
  ...
  pescate_list[[j]] <- pescate
  pescate_list[[j]][is.na(pescate_list[[j]])] <- "NA"
}


for (j in 1:length(pescate_list)) {
  names(pescate_list[[j]]) <- paste(names(pescate_list[[j]]), j, sep="_")
}

# CREATE DATAFRAME
pescate_df_check <- do.call(cbind.data.frame, pescate_list)
colonne_pescata <- grep("^Pescata", names(pescate_df_check), value = TRUE)
pescate_df <- pescate_df_check[, colonne_pescata]

# FORMAT AND CREATE CSV FILE TO USE AS INPUT IN THE HIriplex-S WEBSITE
pescate_df <- data.frame(t(pescate_df))
colnames(pescate_df) = Hps_SNPs$Rs_allele

###### INSERT INDEL GENOTYPE (0/1/2/NA) ######
pescate_df$rs312262906_A <- geno_indel
...
if (geno_imputed != 0) {
  ...
  pescate_df[,rs_imputed]<-geno_imputed[i,"GT"]
  }
}

row.names(pescate_df) <- paste(sampleid, row.names(pescate_df), sep = "_")
output_df <- rownames_to_column(pescate_df)
