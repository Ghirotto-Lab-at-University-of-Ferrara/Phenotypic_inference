...
eye_colour_df$Percent <- (eye_colour_df$Count / sum(eye_colour_df$Count)) * 100
...
# Calcola le percentuali per ciascuna categoria
hair_colour_df$Percent <- (hair_colour_df$Count / sum(hair_colour_df$Count)) * 100
...
# Calcola le percentuali per ciascuna categoria
skin_colour_df$Percent <- (skin_colour_df$Count / sum(skin_colour_df$Count)) * 100
...
ggsave(filename=paste(sample_id, "_DC.pdf", sep = ''), 
       plot = final_plot,
       width = 297, 
       height = 210, 
       units = "mm")
